...
////////////// TODOS LOS PRODUCTOS APUNTEN AL DETALLE DEL PRODUCTO
  const getProduct = (idURL) => { // Este (idURL) id viene del useParams de la url
    return new Promise((resolve, reject) => {
     const reqItem = dataProduct.find(( item )=> { // El metodo find = Cuando el id del producto (id:	6) sea igual al de la url
      console.log(item.id, idURL)
      return  ( item.id === parseInt(idURL)  ) }) // como id es un numero toca parsear el idURL de una url para que funcione
     

      setTimeout(() => {
        reqItem ? resolve(reqItem) : reject("No se encontro el producto")
      }, 2000);
    });
  };
////////////// TODOS LOS PRODUCTOS APUNTEN A LA CATEGORIA DEL PRODUCTO
  const getProductByCategory= (categoryURL) => { 
    return new Promise((resolve, reject) => {
     let reqItems = dataProduct.filter( item => item.category === categoryURL ); // vamos entrar dataProduct vamor ir a cada item y retornar todos los que cumplan con la condicion category que sea igual a la categotyURL
    
     setTimeout(() => {
      reqItems ? resolve(reqItems) : reject("No se encontro el producto")
    }, 2000);
    });
  };
